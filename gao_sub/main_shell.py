...
from other.season_label import leverage_ratio, standard_deviation
from other.season_label import down_std, compute_alpha_categroy, rbsa

from gao_sub.config import Config
...
class RunSeasonLable(object):
    def __init__(self):
        ...
        self.db_insert_session = DBSession(Config.SQL_INDEX_INSERT)
        return
    ...
    def run_down_std(self):
        fund_code = self.get_fund_code_by_filter(Config.DOWN_STD_FILTER)
        run_cnt = 0
        fd_num = len(fund_code)
        logging.info('funds number: {}'.format(fd_num))
        for code in fund_code:
        ...
        for code in fund_code:
            ...
            for date_list in self.run_date_list:
                ...
                for year_lag in range(1, 4):
                    ...
                    start_date = pd.date_range(start=tmp_date, periods=2, freq='D').strftime('%Y%m%d').tolist()[-1]
                    try:
                        rst_value = rbsa(code, start_date, report_date)
                        my_logger.info('start_date: {} end_date: {}'.format(start_date, report_date))
                        my_logger.info('code {} year_lag {} ir{}'.format(code, year_lag, rst_value))
                    except Exception as err:
                        my_logger.warning(err)
                        rst_value = None
                    rec = (
                        code, 'Last{}YearR2'.format(year_lag), str(rst_value), None, self.algid,
                        report_date,
                        self.thisfilename, self.todaydate)
...
class RunSeasonLableChild(RunSeasonLable):
    ...
    def run_alpha2(self):
        fund_code = self.get_fund_code_by_filter(Config.ALPHA_FILTER)
        run_cnt = 0
        fd_num = len(fund_code)
        logging.info('funds number: {}'.format(fd_num))
        for code in fund_code:
