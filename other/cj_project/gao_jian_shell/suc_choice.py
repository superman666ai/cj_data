...
def main(year, code):
    ...
    for i in range(1, 8):
        ...
        try:
            ...
            other_df = predict(datetime.strptime(begin, "%Y-%m-%d").strftime("%Y%m%d"),
                               (datetime.strptime(end, "%Y-%m-%d").strftime("%Y%m%d")), code, stock_index)
