...
class SeasonLabel(object):
    ...
    def init_funcs(self):
        ...
        classes = [LeverageRatio, StandardDeviation, MaxDrawDown, IntervalProfit, AlphaCategroy, SharpRation, DownStd,
                   InfomationRatio, RelativeMaxBack]
...
class RelativeMaxBack(object):
    ...
    def add_func(self):
        # 对外接口
        self.season_lable.relative_maxback = self.relative_maxback
        return

    def relative_maxback(self, code, start_date, end_date, tag=None, type=None):
        """
        ...
        # 按照查找某指数基金净值
        market = self.find_tag_value()

        # 按照日期查找基金净值
        all = self.find_net_value(code, start_date, end_date, type)
    ...
    def market_fetch_Hongkong(self, stock_index):
        """
           功能
           --------
           得到某港股指数的全部收盘价数据
    ...
    def market_fetch_CBA(self, stock_index):
        """
            功能
            --------
            得到某中债指数的全部收盘价数据
    ...
    def get_track_index(self, stock_index):
        """
        功能
        --------
    ...
    20020110  1281.261
        ...
        i = stock_index
        if i == None:
            market = self.market_fetch(self, 'h11009')
        elif i[-3:] in ['.SH', '.SZ', 'CSI', '.MI']:
            if i == 'RMS.MI':
                market = self.get_market_wind(self, i)
            else:
                stock = i[0:6]
                market = self.market_fetch(self, stock)
        elif i[-3:] == '.CS':
            stock = i[0:8]
            market = self.market_fetch_CBA(self, stock)
        elif i[-3:] == '.HI':
            market = self.market_fetch_Hongkong(self, i)
        else:
            market = self.get_market_wind(self, i)
        if market.empty:
            market = self.get_market_wind(self, i)
    ...
    def find_net_value(self, code, start_date, end_date, type):
        """
        查询基金的日净值收盘价
        :param code:
        :param start_date:
    ...
    def find_tag_value(self, tag=None):
        """
        查询某指数的日净值
        :param tag:
        :param start_date:
        ...
        i = tag
        if i == None:
            market = self.market_fetch('h11009')
        elif i[-3:] in ['.SH', '.SZ', 'CSI', '.MI']:
            if i == 'RMS.MI':
                market = self.get_market_wind(self, i)
            else:
...
# 计算相对收益率
relative_maxback = tmp_local_season_label.relative_maxback
